...
function createCountdownTimer(days = 28, hours = 20, minutes = 45, seconds = 20) {
  ...
  timerContainer.className = 'expandable-cross-sell-card-timer';
...
/**
 * Creates the content for the expandable cross-sell card
...
function createContent(config, additionalElements = []) {
  ...
  contentContainer.className = 'expandable-cross-sell-card-content';
  
  // Create heading
  if (config.heading) {
    const heading = document.createElement('h2');
    heading.className = 'expandable-cross-sell-card-heading';
  ...
  if (config.description) {
    ...
    description.className = 'expandable-cross-sell-card-description';
  ...
  if (config.ctaText && config.ctaLink) {
    ...
    ctaButton.className = 'expandable-cross-sell-card-cta';
  ...
  if (config.note) {
    ...
    note.className = 'expandable-cross-sell-card-note';
  ...
  if (additionalElements && additionalElements.length > 0) {
    ...
    additionalContent.className = 'expandable-cross-sell-card-additional-content';
...
/**
 * Processes the cross-sell card content block
...
/**
 * Decorates the expandable cross-sell card block
 * @param {Element} block The block element
 */
export default function decorate(block) {
  ...
  // Find card content blocks
  const contentBlocks = [...block.querySelectorAll('.card-content')];
  let contentConfig = {};
  let contentElements = [];
  
  if (contentBlocks.length > 0) {
    // Process the first content block
    const processedContent = processCardContent(contentBlocks[0]);
  ...
  container.className = 'expandable-cross-sell-card-container';
  
  // Create main block elements
  const cardWrapper = document.createElement('div');
  cardWrapper.className = 'expandable-cross-sell-card-wrapper';
  
  // Add background image if available
  if (backgroundImage) {
    const picture = createOptimizedPicture(backgroundImage.src, backgroundImage.alt, false, [{ width: '2000' }]);
    picture.className = 'expandable-cross-sell-card-background';
  ...
  cardWrapper.appendChild(timer);
  cardWrapper.appendChild(content);
  
  // Add expand/collapse functionality
  const expandButton = document.createElement('button');
  expandButton.className = 'expandable-cross-sell-card-toggle';
  ...
  expandButton.addEventListener('click', () => {
    ...
    const isExpanded = cardWrapper.classList.contains('expanded');
